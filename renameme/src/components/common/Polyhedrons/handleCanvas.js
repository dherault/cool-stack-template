function handleCanvas(canvas, width, height) {
  ...
  const faceColor = 'rgba(59, 130, 246)'
  ...
  const nameToScale = {
    ...
    tetrahedron: 64 - 4,
    octahedron: 64 - 8 - 4,
    icosahedron: 64 - 16 - 8 - 4,
    dodecahedron: 64 - 32 - 8,
  }
  ...
  polyhedronNames.forEach((polyhedronName, i) => {
    polyhedrons.push(createPolyhedronInstance(patrons[polyhedronName], nameToScale[polyhedronName], i))
  })
  ...
  function createPolyhedronInstance(patron, scale, index = 0) {
    const scaleFactor = scale * (isMobile ? 0.75 : 1)
    ...
    return {
      ...
      b: params[1],
      c: params[2],
      da: PI / (512 + 256 + 128 + 64 + 32 + 16 + 8 + 4 + 2),
      db: PI / (512 + 256 + 128 + 64 + 32 + 16 + 8 + 4 + 2),
      dc: PI / (512 + 256 + 128 + 64 + 32 + 16 + 8 + 4 + 2),
      x: width / 2 + (index - 2) * 128,
      y: height / 2,
